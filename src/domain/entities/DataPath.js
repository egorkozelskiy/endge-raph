...
export class DataPath {
    ...
    //
    static _cacheFromString = new Map();
    static _cacheToString = new WeakMap();
    static _cacheSegments = new Map();
    constructor(segs) {
        this._segs = segs;
        //
    ...
    static _interpolate(src, vars) {
        ...
        s = s.replace(/\[([a-zA-Z_$][\w$]*)\s*=\s*(\$[a-zA-Z_$][\w$]*)\]/g, (_m, key, v) => {
            const name = v.slice(1);
            const val = get(name);
            const rendered = typeof val === 'number' || typeof val === 'boolean'
                ? String(val)
        ...
        s = s.replace(/\[\s*(\$[a-zA-Z_$][\w$]*)\s*\]/g, (_m, v) => {
            ...
            if (typeof val !== 'number' || !Number.isFinite(val)) {
                throw new Error(`DataPath: variable "${name}" must be a finite number for index segment`);
            }
            return `[${val}]`;
        });
        ...
        s = s.replace(/(^|\.)(\$[a-zA-Z_$][\w$]*)(?=\.|\[|$)/g, (_m, lead, v) => {
            ...
            if (!/^[a-zA-Z_$][\w$]*$/.test(key)) {
                throw new Error(`DataPath: variable "${name}" must be a valid identifier for key segment`);
            }
            return `${lead}${key}`;
        });
        return s;
    }
    static _parseSegments(path) {
        const segs = [];
        const re = /([^\.\[\]]+)|\[(.+?)\]/g;
        let m;
        while ((m = re.exec(path)) !== null) {
            ...
            if (kv) {
                ...
                const quoted = (rawVal.startsWith('"') && rawVal.endsWith('"')) ||
                ...
                const pval = !quoted && /^\d+$/.test(rawVal) ? Number(rawVal) : rawVal;
    ...
    toPlain() {
        ...
        const deepOnTail = this._segs.length > 0 &&

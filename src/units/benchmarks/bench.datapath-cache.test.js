...
describe('bench.datapath-cache', () => {
    ...
    it('использует кэш сегментов при повторном разборе', () => {
        ...
        console.log(`[bench.datapath-cache] parse_cached=${(t1 - t0).toFixed(4)}ms, toString_cached=${(t3 - t2).toFixed(4)}ms`);
    });
    it('значительно быстрее при повторных вызовах fromString', () => {
        ...
        const cold = t1 - t0;
        const hot = t3 - t2;
        console.log(`[bench.datapath-cache] cold=${cold.toFixed(2)}ms, hot=${hot.toFixed(2)}ms, speedup=${(cold / hot).toFixed(2)}x`);
        expect(hot).toBeLessThan(cold * 0.5); // кэш должен быть как минимум в 2 раза быстрее
    });
});
//# sourceMappingURL=bench.datapath-cache.test.js.map